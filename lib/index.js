...
class VuexRender extends Render {
    ...
    render() {
        const { props, slots, events, methods, mixIns, data, computed, watch, getters, actions, mutations, state } = this.parserResult;
        let md = {};
        if (getters) {
...
function parseJavascript(ast, options = {}) {
    traverse__default(ast, {
        ExportDefaultDeclaration(rootPath) {
            ...
            rootPath.traverse({
                ObjectProperty(path) {
                    const { onProp, onMethod, onComputed, onName, onSlot, onMixIn, onData, onWatch, onGetter, onAction, onMutation, onState } = options;
...
cli
  .command('genagain', 'Generate target resources')
  .allowUnknownOptions()
  .action(async flags => {
    ...
    let filename = path.resolve('website/index.html');
    const indexSource = await fs.readFile(filename, 'utf-8'),
